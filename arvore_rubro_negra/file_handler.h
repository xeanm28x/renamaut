...
void gravar_arvore(FILE *file, MaquinaAutonoma *arvore)
{
    ...
    fprintf(file, "%s|%s|%s|%s|%s|%d|%s|%d|%s|%s\n",
            ...
            arvore->responsavel,
            arvore->status,
            arvore->localizacao->cidade,
            arvore->localizacao->uf);

    gravar_arvore(file, arvore->esq);
    gravar_arvore(file, arvore->dir);
}
